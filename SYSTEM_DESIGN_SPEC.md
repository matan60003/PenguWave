...
│   │   ├── database/             # Data Access Layer: (e.g., models.py, repositories.py) ORM setup and abstracted queries
│   │   ├── schemas/              # Validation Layer: (e.g., schemas.py) Pydantic schemas for serialization
│   │   └── services/             # Business Logic Layer: (e.g., auth_service.py) Core algorithms & DI injection
│   └── tests/                    # Automated Pytest suite (integration & unit)
└── docker-compose.yml            # Infrastructure orchestration
```

### Layer Responsibilities in Practice
*   **`app/api` (Routers):** Exclusively handles HTTP request parsing, routing, and Dependency Injection for services.
*   **`app/schemas` (Pydantic):** Defined in `schemas.py`. Enforces data shape. If the frontend sends an invalid string for an email, Pydantic throws a `422 Unprocessable Entity`.
*   **`app/services` (Business Logic):** E.g., `AuthService`. Contains the actual algorithmic work, decoupled from HTTP and raw Database contexts.
*   **`app/database/repositories.py` (Data Access):** The Repository Pattern. Abstracts raw SQLAlchemy session queries (e.g., `UserRepository.get_by_email()`) away from the service layer.
...
2.  **Routing & Auth Gate (`app/api/events.py`):** 
    ...
    async def get_events(
        ...
        return await event_service.get_events(severity=None, search=search, limit=limit, offset=offset)
    ```

3.  **Business Logic (`app/services/event_service.py`) & Data Access (`app/database/repositories.py`):** 
    The service layer delegates the dynamic SQLAlchemy query to the repository layer inside PostgreSQL.
    ```python
    class EventRepository:
        async def get_events(self, severity, search, limit, offset):
            ...
            result = await self.db.execute(query.order_by(models.Event.timestamp.desc()).offset(offset).limit(limit))
...
3.  **Concurrency Control (Advisory Locks):**
    Before executing the HTTP request, the scheduler calls `pg_try_advisory_lock` via SQLAlchemy. If multiple server workers wake up simultaneously, the database grants the lock to only one. The others instantly fail to acquire the lock and gracefully skip the cycle. This ensures high availability and eliminates duplicate data ingestion without relying on Redis or Celery.
