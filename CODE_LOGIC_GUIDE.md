...
*Where it lives:* `backend-service/app/core/security.py` & `backend-service/app/api/dependencies.py`

### Important Functions:
*   **`create_access_token(data: dict)`**: Located in `security.py`. When a user successfully logs in, this function takes their user ID, adds an expiration time (`exp`), and signs it cryptographically using our `SECRET_KEY`. This outputs the JWT string.
...
## 2. Database Services (The Business Logic)
*Where it lives:* `backend-service/app/services/event_service.py`

### Important Function:
*   **`get_events(severity, search, limit, offset, db)`**: This is the heart of our **Server-Side Pagination**.
    *   **The Logic:** Instead of just returning all events, it starts with a base query (`db.query(models.Event)`). 
    ...
    *   It calculates the `total` number of matching rows using `.count()`.
    *   Finally, it applies the `.offset(offset).limit(limit)` logic to slice out exactly 25 rows, preventing our database from sending massive payloads.
...
*   **`_fetch_and_ingest_cisa_data()`**: Located in `scheduler.py`.
    ...
    *   **Deduplication Logic:** Before saving, it runs `db.query(models.Event).filter(models.Event.title == title).first()`. If it finds a match, it skips it (`continue`). This ensures we don't accidentally insert the same vulnerability twice every 5 minutes.
