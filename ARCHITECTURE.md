...
- **FastAPI (Python):**
  ...
  - **Why it matters:** FastAPI uses asynchronous programming (`async`/`await`), meaning it can handle many concurrent requests extremely fast without slowing down. Coupled with our asynchronous database layer (`asyncpg`), it never blocks the main event loop.
...
- **Clean Architecture & Repository Pattern:**
  - **What it is:** A software design philosophy separating concerns into isolated layers (API, Service, Database).
  - **How we use it:** We use the Repository Pattern (e.g., `UserRepository`, `EventRepository`) to abstract SQLAlchemy database queries, and we inject service classes (e.g., `AuthService`, `UserService`, `EventService`) into our FastAPI routes using Dependency Injection.
  - **Why it matters:** It strictly decouples business logic from frameworks. The API router knows nothing about SQLAlchemy, making the code vastly easier to test and maintain.
...
- **SQLAlchemy 2.0 (Async ORM):**
  - **What it is:** An Object-Relational Mapper for Python with full asynchronous support.
  - **How we use it:** Instead of writing raw SQL strings, SQLAlchemy allows us to interact with the database using standard Python objects and the asynchronous `select()` syntax (e.g., `await db.execute(select(Event))`).
  - **Why it matters:** It makes the code vastly cleaner, safer from SQL Injection attacks, and by utilizing `AsyncSession`, it ensures that database operations don't block the FastAPI server's main thread. We also implemented strict "Rollback" strategies, meaning if an error happens during a database operation, SQLAlchemy cleanly reverts the changes so the database connection isn't broken.
...
- **Distributed Leader Election (Advisory Locks):**
  - **What it is:** A lightweight, connection-bound lock managed directly by PostgreSQL.
  - **How we use it:** Before our background scheduler fetches data, it attempts to acquire a lock (`pg_try_advisory_lock`). If it succeeds, it fetches and ingests the data. If it fails, it skips the cycle.
  - **Why it matters:** It prevents concurrency bugs. If we scale the FastAPI backend to multiple workers, all workers will try to run the scheduler at the same time. The advisory lock ensures only one worker (the "leader") performs the task, preventing race conditions and redundant database inserts without needing an external message broker like Redis.
